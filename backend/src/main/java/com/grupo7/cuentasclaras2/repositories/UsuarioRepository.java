...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
        ...
        @Query("SELECT u FROM Usuario u " +
                        "WHERE LOWER(u.username) LIKE LOWER(CONCAT('%', :usernameQuery, '%')) " +
                        "AND u.id <> :id " +
                        "AND u IN (SELECT a FROM Usuario usuario JOIN usuario.amigos a WHERE usuario.id = :id) " +
                        "AND NOT EXISTS (SELECT 1 FROM Grupo g JOIN g.miembros gu WHERE g.id = :groupId AND u.id = gu.id) "
                        +
                        "AND u NOT IN (SELECT i.destinatario FROM Invitacion i WHERE i.grupo.id = :groupId)")
        List<Usuario> findFriendsByUsernameNotInGroup(@Param("usernameQuery") String usernameQuery,
                        @Param("id") long id,
        ...
        @Query("SELECT u FROM Usuario u " +
                        "WHERE LOWER(u.username) LIKE LOWER(CONCAT('%', :usernameQuery, '%')) " +
                        "AND u.id <> :id " +
                        "AND NOT EXISTS (SELECT 1 FROM Usuario usuario JOIN usuario.amigos a WHERE usuario.id = :id AND u.id = a.id) "
                        +
                        "AND NOT EXISTS (SELECT 1 FROM Grupo g JOIN g.miembros gu WHERE g.id = :groupId AND u.id = gu.id) "
                        +
                        "AND u NOT IN (SELECT i.destinatario FROM Invitacion i WHERE i.grupo.id = :groupId)")
        List<Usuario> findNonFriendsByUsernameNotInGroup(@Param("usernameQuery") String usernameQuery,
                        @Param("id") long id,
                        @Param("groupId") long groupId);
